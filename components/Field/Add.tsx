...
const schema = yup.object({
  label: yup.string().required('Please enter a name').max(255, 'Name is too long'),
  type: yup.string().default(''),
  required: yup.bool().default(true),
});

const FIELD_TYPES = ['text', 'email', 'textarea', 'number', 'checkbox'];
...
const FieldAdd = ({ children, formId, onAdd }: FieldAddProps) => {
  ...
  if (!formId) {
    spawnFlash('Form ID is missing', 'error');
    return null;
  }
      ...
      <Modal open={showModal} setOpen={setShowModal}>
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="mt-4">
            ...
            <label className="block text-sm text-gray-400" htmlFor="keyField">
              Key
            </label>
            <input
              value={key}
